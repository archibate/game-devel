...
struct Sphere {
	...
	double intersect2(const Ray &r) const { // returns distance, 0 if nohit 
		Vec op = p-r.o; // Solve t^2*d.d + 2*t*(o-p).d + (o-p).(o-p)-R^2 = 0 
		double t, eps=1e-4, b=op.dot(r.d), det=b*b-op.dot(op)+rad*rad; 
		if (det<0) return 0; else det=sqrt(det); 
		return (t=b-det)>eps ? t : ((t=b+det)>eps ? t : 0); 
	} 
	double intersect(const Ray &r) const { // returns distance, INFINITY if nohit
...
Sphere scene[] = {//Scene: radius, position, emission, specular, diffuse 
#if 1
	Sphere(1e5, Vec( 1e5+50,40.8,-81.6), Col(.75,.25,.25),Col(1,1,1)*.9,Col()),//Left 
	Sphere(1e5, Vec(-1e5-50,40.8,-81.6), Col(.25,.25,.75),Col(1,1,1)*.9,Col()),//Rght 
	...
	Sphere(8.8 ,Vec(-30, 4.8,-77),Col(), Col(1,1,1)*.0, Col(1,1,1)*.5),//Ball 
	Sphere(8.8 ,Vec(-30,44.8,-77),Col(), Col(1,1,1)*.0, Col(1,1,1)*.5),//Ball 
#else
...
Col radiance(const Ray &r, int maxRefl)
{
	...
	Vec nl(dn<0?n:-n);
	Col cl = s.e*-dn;
	if (maxRefl-- > 0 &&s.spec != Col()) {
		Ray rr(x, ~(r.d-(n*2)*dn));
		cl += radiance(rr, maxRefl)*s.spec;
	}
	if (maxRefl-- > 0 &&s.diff != Col()) {
		cl -= diffuse(r,x,n, 300, maxRefl)*dn*s.diff;
	}
	return cl;
}
...
Col *fisheye(Col *pixels, int &w, int &h, double R=1.414)
{
	...
	for (int y=0;y<h;y++){const double sy=(double(h-1-y*2)/(h-1));
		...
		for (int x=0;x<w;x++){const double sx=(double(x*2+1-w)/(w-1));{
			...
			for(int inx=nx;inx/*==nx*/>pnx;inx--){int pny=pnya[nx];double
				yfacdif=1/double(ny-pny-1);double yfac=1;for(int
				iny=ny;iny/*==ny*/>pny;iny--,yfac-=yfacdif){newPixels[inx+
...
#define DFLARG(i,v) (argc>i?argv[i]:(v))
	...
	int w=300,h=w;
	const double edw=0.0, fow=1.0/*=0.5135*/, fshr=1e5;//;(fow>0.7?2/(fow-.5):fow>0.3?3/(fow-.2):1e5);
	/*Ray cam(Vec(0,0,0), ~Vec(0,0,-1));
	Vec cx(fow,0,0), cy(~(cx^cam.d)*fow), cz(cam.d);*/
	Col *pixels = new Col[w*h];
#pragma omp parallel for schedule(dynamic, 1)
	for (int y=0;y<h;y++){const double ay(double(h-1-y*2)/(h-1)*fow);
		fprintf(stderr,"\rRendering %5.2f%%...",100.*y/(h-1));
		for (int x=0;x<w;x++){const double ax(double(x*2+1-w)/(w-1)*fow);{
#define pow2(x) ({typeof(x)_x_=(x);_x_*_x_;})
			/*Vec da(sin(ax)*cos(ay/2*fow),sin(ay)*cos(ax/2*fow),cos(ax)*cos(ay)/sqrt(pow2(cos(ax))+pow2(cos(ay)))/);
			Vec d(~(cx*da.x+cy*da.y+cz*da.z));
			Ray sr(cam.o-d*edw,d);*/
			Vec d(sin(ax),sin(ay),-cos(ax)*cos(ay));d=~d;Ray sr(-d*edw,d);
			//printf("(%f, %f, %f)\n", XYZ(sr.d));
