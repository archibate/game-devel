...
class AntStateDelivering(State):
    ...
    def check_conditions(self):
        if Vector2(*NEST_POSITION).get_distance(self.ant.location) < NEST_SIZE:
            if (randint(1, 10) == 1):
...
def run():
    ...
    leaf_image = pygame.image.load("leaf.bmp").convert_alpha()
    spider_image = pygame.image.load("spider.bmp").convert_alpha()
